...
public class Gauss implements Solver {
	...
	private void nullify(int row, int col) throws Exception {
		...
		// multiplyRowBy(row, nextCoefficient);
		// subtractRow(row, row + 1, thisCoefficient);
	...
	private void printState() {
		for (int i = 0; i < coefficients.length; i++) {
			for (int j = 0; j < coefficients.length; j++) {
				System.out.printf("%.2f\t", ((R) coefficients[i][j]).r);
			}
			System.out.printf("%.2f%n", ((R) independent[i]).r);
		}
		System.out.println();
	}
}
